...
```{python}
## TODO for Isabel: can you add the `inspection_date` predictor to this random forest model?
## TODO for Isabel: can you add a script to `data-raw` that trains this model and deploys to Connect? see my `prep_chicago_inspections.R` but *only* do model training, not data prep
import pandas as pd
import numpy as np
from sklearn import preprocessing, tree, pipeline, compose
import pins

pd.read_parquet('./data/inspections.parquet', engine = 'pyarrow')
categorical_features = ['facility_type', 'risk']
X, y = inspection.drop(columns=['aka_name', 'results']), inspection['results']
...
```python
## TODO for Isabel: put a model on Colorado using the script in `data-raw` and run this for real
...
```python
## TODO for Isabel: update this with real URL but we will not run (just here for auto-animate)
from vetiver.server import predict, vetiver_endpoint
endpoint = vetiver_endpoint(f"https://connect.example.com/content/{APP_ID}/predict")
